...
做了一个开源工具叫 ai4wechat，专门解决这个问题。核心思路是做一个桥接层，你的 AI 服务不用改任何代码。
...
`conversation_id` 按用户稳定，你可以在服务端基于它做多轮对话。

你的 AI 服务完全不知道用户是从微信来的还是从网页来的，它只看到一个标准的 HTTP 请求。
...
|---|---|---|
| 文本 | 稳定 | 稳定 |
| 图片 | 元数据 | 规划中（CDN 协议已验证） |
| 文件 | 元数据 | 规划中（CDN 协议已验证） |
| URL | — | 支持（微信自动识别） |
| Emoji | — | 支持 |

图片和文件的 CDN 上传流程（AES-128-ECB 加密 → CDN 上传 → sendmessage）已经在协议层面验证通过了，SDK 实现还在路线图里。
...
浏览器打开 `http://服务器IP:18891` 扫码。凭证保存后不用重复扫。

HTTP 超时默认 120 秒。推理慢的模型可以 `--timeout 300` 调大。

## 项目地址

GitHub: https://github.com/ai4wechat/ai4wechat
